...
Dialog::AddToPlaylist::AddToPlaylist(lib::spt::api &spotify,
	lib::spt::playlist playlist, std::vector<std::string> trackIds, QWidget *parent)
	: Base(parent),
	spotify(spotify),
...
void Dialog::AddToPlaylist::showEvent(QShowEvent *event)
{
	spotify.playlist_tracks(playlist,
		[this, event](const std::vector<lib::spt::track> &tracks)
		{
			playlistTrackIds.clear();
			for (const auto &track: tracks)
			{
				playlistTrackIds.insert(track.id);
			}
